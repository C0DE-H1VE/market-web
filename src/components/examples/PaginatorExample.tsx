...
export default function PaginatorExample() {
  ...
  return (
    <div className="p-6 border rounded-lg bg-white shadow-sm space-y-6">
      <h3 className="text-xl font-semibold text-gray-800">📄 Sistema de Paginação</h3>
      ...
      <div className="space-y-4">
        ...
        <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
          ...
          <div className="mt-2 text-xs space-y-1">
            {totalPages <= 5 && <p>• ≤5 páginas: Mostra todas (← 1 2 3 4 5 →)</p>}
            {totalPages === 6 && <p>• 6 páginas: Uma reticência (← 1 2 3 4 ... 6 →)</p>}
            {totalPages > 6 && currentPage <= 3 && <p>• Início: ← 1 2 3 ... {totalPages} →</p>}
            {totalPages > 6 && currentPage > 3 && currentPage < totalPages - 2 && 
              <p>• Meio: ← 1 ... {currentPage} ... {totalPages} → (7 elementos)</p>
            }
            {totalPages > 6 && currentPage >= totalPages - 2 && 
              <p>• Fim: ← 1 ... {totalPages-2} {totalPages-1} {totalPages} →</p>
            }
          </div>
        </div>
      </div>
      ...
      <div className="border-t pt-4">
        ...
        <div className="mt-4 p-3 bg-gray-100 rounded text-xs">
          ...
          <p>← (1) + ... (1) + página atual (1) + ... (1) + última (1) + → = 7 elementos</p>
        </div>
      </div>
    </div>
  )
